...
namespace HomeWork2020Debtor
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            #region  2) rhyta.com ve ya dayrep.com domenlerinde emaili olan borclulari cixartmag

            var result2 = debtorsList.FindAll(x => x.Email.Contains("rhyta.com") || x.Email.Contains("dayrep.com "));
            ...
            #endregion
            #region 3) Yashi 26-dan 36-ya qeder olan borclulari cixartmag

            var result3 = debtorsList.FindAll(x => DateTime.Now.Year - x.BirthDay.Year >= 26 && DateTime.Now.Year - x.BirthDay.Year <= 36);



            #endregion
            #region 4)Borcu 5000-den cox olmayan borclularic cixartmag

            var result4 = debtorsList.FindAll(x => x.Debt <= 5000);


            #endregion
            #region 5) Butov adi 18 simvoldan cox olan ve telefon nomresinde 2 ve ya 2-den cox 7 reqemi olan borclulari cixartmaq
            var result5 = debtorsList.FindAll(x => x.FullName.Length > 18 && x.Phone.Count(j => j == 7) >= 2);

            #endregion
            #region 7) Qishda anadan olan borclulari cixardmaq 
            var result7 = debtorsList.FindAll(x => x.BirthDay.Month == 12 || x.BirthDay.Month == 1 || x.BirthDay.Month == 2);


            LoopList(result7);
            #endregion
            #region 8) Borcu, umumi borclarin orta borcunnan cox olan borclulari cixarmaq ve evvel familyaya gore sonra ise meblegin azalmagina gore sortirovka etmek

            #endregion
            #region 9) Telefon nomresinde 8 olmayan borclularin yalniz familyasin,yashin ve umumi borcun meblegin cixarmaq
            var result9 = debtorsList.FindAll(x => x.Phone.Contains('8') == false);

            foreach (var item in result9)
            {
                Console.WriteLine($"FullName ==> {item.FullName} ==> Age:{DateTime.Now.Year - item.BirthDay.Year} ==> Debtor:{item.Debt}");
            }
            #endregion
            #region 11)Adinda ve familyasinda hec olmasa 3 eyni herf olan borclularin siyahisin cixarmaq ve onlari elifba sirasina gore sortirovka elemek
            List<Debtor> debtors = new List<Debtor>();
            for (int i = 0; i < debtorsList.Count; i++)
            {
                Dictionary<char, int> keyValuePairs = new Dictionary<char, int>();

                foreach (var item in debtorsList[i].FullName)
                {
            ...
            #endregion
            #region 	13)borclulardan en coxu hansi ilde dogulubsa hemin ili cixartmaq
                         Result13(debtorsList);

            #endregion
            #region 14)Borcu en boyuk olan 5 borclunun siyahisini cixartmaq
            var result14 = debtors.OrderByDescending(x => x.Debt).Take(5);

            LoopList(result14.ToList());
            #endregion
            #region 15)Butun borcu olanlarin borcunu cemleyib umumi borcu cixartmaq
            var result15 = debtorsList.Where(x => x.Debt > 0);
            var count15 = 0;
            ...
            Console.WriteLine($"Debt:{count15}");
            #endregion
            #region 	16)2ci dunya muharibesin gormush borclularin siyahisi cixartmaq
            var result16 = debtorsList.FindAll(x => x.BirthDay.Year <= 1945);
            LoopList(result16);
            #endregion
            #region 18)Nomresinde tekrar reqemler olmayan borclularin ve onlarin borcunun meblegin cixartmaq

            #endregion
            #region 19)Tesevvur edek ki,butun borclari olanlar bugunden etibaren her ay 500 azn pul odeyecekler. Oz ad gunune kimi borcun oduyub qurtara bilenlerin siyahisin cixartmaq
            int count;
            List<Debtor> result19 = new List<Debtor>();
            ...
            foreach (var item in debtors)
            {
                ...
                {
                    count = item.BirthDay.Month - DateTime.Now.Month;
                }
                else
                {
                    count = DateTime.Now.Month - item.BirthDay.Month;
                }

                if (count * 500 >= item.Debt)
                {
                    result19.Add(item);
                }
            }
            ...
            LoopList(result19);
            #endregion
            #region 20)Adindaki ve familyasindaki herflerden "smile" sozunu yaza bileceyimiz borclularin siyahisini cixartmaq
            var result20 = debtors.FindAll(d => d.FullName.ToLower().Contains('s') && d.FullName.ToLower().Contains('m') && d.FullName.ToLower().Contains('i') && d.FullName.ToLower().Contains('l') && d.FullName.ToLower().Contains('e'));

            LoopList(result20.ToList());
            #endregion
        }

        static void Result13(List<Debtor> list)
        {
            ...
            for (int i = 0; i < list.Count; i++)
            {
                ...
                int BirthDayYear = list[i].BirthDay.Year;
                if (keyValuePairs.ContainsKey(BirthDayYear))
                {
                    keyValuePairs[BirthDayYear]++;
                }
                else
                {
                    keyValuePairs.Add(BirthDayYear, 1);
                }
        ...
        static void LoopList(List<Debtor> debtors2)
            {
                Console.WriteLine("\a                                              Result =================>");
                foreach (var item in debtors2)
                {
                    Console.WriteLine(item);
                    Console.WriteLine("\n");
